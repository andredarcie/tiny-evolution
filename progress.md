...
- 2026-04-07: adicionado indicador visual sutil para suporte de colônias terminais em `TurnGameScene`: halo verde discreto na colônia terminal e badge `+N` nos vizinhos estabelecidos que receberão biomassa na Seleção Natural. Validação: `npx tsc --noEmit`, `npm run build` e captura Playwright básica passaram; a captura curta valida renderização geral, mas não alcança naturalmente um estado terminal.
- 2026-04-07: adicionada animação de ganho de biomassa: ganhos positivos criam uma bolinha verde que sai da colônia e voa até o bloco de biomassa do HUD; o número exibido no HUD só incorpora o ganho quando a animação chega. Validação: `npx tsc --noEmit` e `npm run build` passaram. `npm run test -- src/tests/TurnGameScene.test.ts` ainda falha por expectativas antigas de progressão/produção já divergentes do estado atual; não houve erro de runtime da animação. Captura Playwright curta passou e confirmou o loop de ganho de biomassa.
- 2026-04-07: investigação de lentidão no celular: achados principais foram canvas usando `devicePixelRatio` sem limite e `TurnGameScene.update()` chamando `updateHUD()` todo frame, causando DOM/innerHTML/chart redraw contínuo. Otimizações aplicadas: cap de renderScale do canvas em 1.5 no mobile e 2 no desktop, remoção do update de HUD por frame, e remoção de blur da topbar em mobile. Validação: `npx tsc --noEmit`, `npm run build` e captura Playwright básica passaram.
